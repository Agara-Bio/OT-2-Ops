...
To each of the 96 wells add:
    90uL CaCl2 (0.25 M) and
    MgCl2 (0.25 M), final concentration 10 mM.
...
def run(PHiTS_1: protocol_api.ProtocolContext):
    ...
    # Labware
    deep_well_plate = protocol.load_labware('nest_96_wellplate_2ml_deep', 6)
    ...
    # Step 1: Add 90uL CaCl2 and MgCl2 to each well // need to split this step
    p300.pick_up_tip()
    ...
    for well in deep_well_plate.wells():
        i = 0
        while i < 2
            p300.aspirate(250, host_medium)
            p300.dispense(250, well)
            i += 1
        p300.mix(3, 400, well)  # Mix by pipetting up and down 3 times
    p300.drop_tip()
